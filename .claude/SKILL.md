...
3. 如果都找不到，提示用户提供 API Key，并引导用户前往 灵客AI 官网（https://lingkeai.vip）登录后在「API 密钥管理」中创建
...
3. **余额不足时**，提示用户前往 灵客AI 官网（https://lingkeai.vip）充值算力
4. **API Key 错误时**，提示用户前往 灵客AI 官网（https://lingkeai.vip）登录并在「API 密钥管理」中创建或检查密钥
